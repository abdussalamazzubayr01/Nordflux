...
if (process.env.MONGODB_URI) {
  ...
  console.log('MONGODB_URI not set. Using fallback file order store:', ORDERS_FALLBACK_FILE);
}
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: 'Thank You For Your Order - ' + displayOrderId(order),
      html: `
        <h2>Thank You! Your Order Was Received</h2>
        <p>Hi ${normalizedCustomerName},</p>
        <p>Thank you for your order. We have received your payment and your order is now being made.</p>
        
        <div style="background: #d4af37; border-radius: 10px; padding: 20px; margin: 20px 0; text-align: center;">
          <p style="color: #1a1a2e; font-size: 12px; margin: 0 0 10px 0; text-transform: uppercase; letter-spacing: 1px;">Your Order ID</p>
          <p style="color: #1a1a2e; font-size: 32px; font-weight: 700; margin: 0; letter-spacing: 2px;"><strong>${displayOrderId(order)}</strong></p>
          <p style="color: #1a1a2e; font-size: 12px; margin: 10px 0 0 0;">Keep this order reference for support updates</p>
        </div>
        ...
        <p>We will keep updating you by email as your order moves through each stage: being made, shipped, coming, and arrived.</p>
        <p><a href="http://${req.get('host')}/html/profile.html" style="color: #007bff; text-decoration: none;"><strong>View In Your Profile</strong></a></p>
        <p>Best regards,<br/>NORDLUXE Team</p>
      `
    };
    ...
    try {
      ...
      console.error('Order confirmation email send error:', emailErr.message);
    }
